...
func init() {
	...
	flag.StringVar(&actuatorImage, "actuator-image", "gcr.io/k8s-cluster-api/libvirt-machine-controller:0.0.1", "Actuator image to run")
...
func (f *Framework) UploadDockerImageToInstance(image, targetMachine string) error {
	...
	cmd := exec.Command("bash", "-c", fmt.Sprintf(
		"docker save %v | bzip2 | ssh -i %v ec2-user@%v \"bunzip2 > /tmp/tempimage.bz2 && sudo docker load -i /tmp/tempimage.bz2\"",
		image,
		f.SSHKey,
		targetMachine,
	))
